...
biomasses$ID_old <- as.factor(biomasses$ID_old)
swimtime$ID <- as.factor(swimtime$ID)

summ_speed <- summary %>% 
  group_by(ID) %>% 
  summarize(speed_min = min(speedBL),
            speed_max = max(speedBL)) %>%
  left_join(biomasses, by = c("ID" = "ID_old")) %>% 
  mutate(ID = ID.y) %>% 
  select(ID, speed_min, speed_max)

align(flextable(biomasses %>%
            left_join((swimtime %>% select(ID, time_d, distance)), by = c("ID_old" = "ID")) %>%
            left_join(summ_speed, by = "ID") %>%
            select("ID", "Length (cm)", "Silvering Index*", "Biomass at start (g)", "Biomass at end (g)", "Fat at start (% WM)", "Fat at end (% WM)", "Silvering Index*", "time_d", "distance", "speed_min", "speed_max") %>%
