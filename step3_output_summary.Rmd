...
summ_speed <- summary %>% 
  group_by(ID) %>% 
  summarize(speed_min = min(speedBL),
            speed_max = max(speedBL),
            temp_min = min(temp_mean),
            temp_max = max(temp_max)) %>%
  left_join(biomasses_temp, by = c("ID" = "ID_old")) %>% 
  mutate(ID = ID.y) %>% 
  select(ID, speed_min, speed_max, temp_min, temp_max)

align(flextable(biomasses %>%
            left_join((swimtime %>% select(ID, time_d, distance)), by = c("ID_old" = "ID")) %>%
            left_join(summ_speed, by = "ID") %>%
            select("ID", "Length (cm)", "Silvering Index*", "Biomass at start (g)", "Biomass at end (g)", "Fat at start (% WM)", "Fat at end (% WM)", "Silvering Index*", "time_d", "distance", "speed_min", "speed_max", "temp_min", "temp_max") %>%
            mutate(time_d = round(time_d, 1),
                   ...
                   "Temperature range (°C)" = paste(temp_min, temp_max, sep = " - ")) %>% 
            rename("Swim time (d)" = time_d,
                   "Distance (km)" = distance) %>% 
              select(-speed_min, -speed_max, -temp_min, -temp_max)) %>% 
...
flextable(summ_by_ID %>%
            ...
            ungroup() %>% 
            select("Nominal Temperature (°C) / Pressure (bar)" , resp_mean, temp_mean, speed_mean) %>%
...
fig1a <- tag_facet2(
  ...
  geom_line(aes(x= ID_time, y=predict(lme.plot, level = 0)), linewidth = 0.4)+
