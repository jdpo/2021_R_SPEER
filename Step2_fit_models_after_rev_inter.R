...
#save full image
save.image(file='summary_mod_full_inter.RData')

#save smaller image
rm(list = ls()[ls() %in% c("lme.dredge" , "lme.final", "lme.i", "lme.p", "lme.s", "lme.t", "fig1", "fig2", "lme.norun", "lme.run", "plot_experiment", "plot_temp", "summary_backup", "summary_raw1", "summary_raw2", "summary_corr")])
save.image(file='summary_mod_inter.RData')

################################ SOME GRAPHS TO CHECK (don't run rm(list...) if needed) ###########################################
